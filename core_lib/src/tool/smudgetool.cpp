...
void SmudgeTool::loadSettings()
{
    ...
    if (mSettings->requireMigration(settings, ToolSettings::VERSION_1)) {
        mSettings->setBaseValue(STROKE_WIDTH_VALUE, settings.value("smudgeWidth", 24.0).toReal());
        mSettings->setBaseValue(STROKE_FEATHER_VALUE, settings.value("smudgeFeather", 48.0).toReal());
...
void SmudgeTool::drawStroke()
{
    ...
    qreal opacity = 1.0;
    mCurrentWidth = width();
    qreal brushWidth = mCurrentWidth + 0.0 * feather();
    qreal offset = qMax(0.0, mCurrentWidth - 0.5 * feather()) / brushWidth;
